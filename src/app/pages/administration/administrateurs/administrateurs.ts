...
@Component({
    ...
    imports: [TableModule, ButtonModule, TagModule, CommonModule, DialogModule, ReactiveFormsModule, InputTextModule, SelectModule, ConfirmDialogModule, ToastModule],
...
export class Administrateurs implements OnInit {
    ...
    async saveAdministrateur(): Promise<void> {
        try {
            ...
            if (this.dialogMode === 'edit' && this.selectedAdministrateur) {
                ...
                await this.supabaseService.updateData('users', userData, this.selectedAdministrateur.id.toString());
                ...
                if (formValue.email !== this.selectedAdministrateur.email && this.selectedAdministrateur.auth_id) {
                    const { error: authError } = await this.supabaseService.client.auth.admin.updateUserById(this.selectedAdministrateur.auth_id, { email: formValue.email });
                ...
                this.toastService.showSuccess('Succès', `Administrateur "${formValue.prenoms} ${formValue.nom}" modifié avec succès`);
                ...
                // 1. Créer l'utilisateur dans Supabase Auth
                const { data: authData, error: authError } = await this.supabaseService.signUp(formValue.email, temporaryPassword);
                ...
                // 3. Afficher un message de succès
                this.toastService.showSuccess('Succès', `Administrateur "${formValue.prenoms} ${formValue.nom}" créé avec succès`);
    ...
    async getAdministrators() {
        try {
            ...
            const { data, error } = await this.supabaseService.client.from('users').select('*').eq('role', 'super_admin');
            ...
            this.tabAdministrators = Array.isArray(data)
                ? data.map((item: any) => ({
                      id: item.id,
